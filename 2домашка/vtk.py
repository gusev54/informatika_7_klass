...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        F = np.zeros(len(self.nodes[0]))
        CM = max(self.nodes[2, :])/2
        for i in range(0, len(self.nodes[0])):
            for j in range(0, len(self.nodes[0])):
                if abs(self.nodes[2, i]-CM) < abs(self.nodes[2, j] - CM):
                    F[i] += 1*(self.nodes[2,j] - CM)  *  abs(self.nodes[2,j] - CM)
            if i % round(len(self.nodes[0])/10) == 0:
                print(round(i*100 / len(self.nodes[0])), '%')
        self.smth = F
        # Тут может быть скорость, но сейчас здесь нули
        RM = max(self.nodes[1, :])/2
        V = 15
        self.velocity = np.zeros(shape=(3, int(len(nodes_coords) / 3)), dtype=np.double)
        # self.velocity[0] = np.power(self.nodes[1] - rm[0], 2)
        self.velocity[1] = -V*(self.nodes[2] - CM)
        self.velocity[2] = V*(self.nodes[1] - RM)
    ...
    def move(self, tau):
        ...
        self.velocity[1] = -V*(self.nodes[2] - CM)*w
        self.velocity[2] = V*(self.nodes[1] - RM)*w
        self.smth *= 0.995
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("nozh" + str(snap_number) + ".vtu")
        writer.Write()
...
tau = 0.01
w = 0.96
RM = max(mesh.nodes[1, :])/2
CM = max(mesh.nodes[2, :])/2
